...
int main()
{
	...
	wchar_t iconName[] = L"logo.ico";								// *.ico файл
	...
	int toCLEAR_M[] = { VK_LWIN, VK_ESCAPE };
	...
	keysComb toCLEAR = { sizeof(toCLEAR_M) / sizeof(toCLEAR_M[0]), toCLEAR_M, pm_CLEAR };
	...
	while (!isExit)
	{
		if (IsKeysPressed(toSWAP) || IsKeysPressed(toUPDOWN) || IsKeysPressed(toCLEAR))
		{
			//GET CONSERVATION;
